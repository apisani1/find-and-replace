...
function tests:cov {
    ...
    TEST_FILE="${1:-$(get:python:files:tests)}"
    shift || true
    poetry run pytest "$TEST_FILE" --cov=find_and_replace  --cov-report=term "$@"
}
...
function docs:api {
    ...
    cd docs && poetry run sphinx-apidoc -o api ../src/find_and_replace -f
}
...
function help {
    echo "$0 <task> <args>"
    echo ""
    echo "====== find-and-replace Development Tool ======"
    echo ""
    echo "Environment:"
